...
subpopEstimate <- function(finalData,parameter, subpopulationCategory, subpopulation, altName){
  ...
  design.ext <- mutate(subpopData,siteID = siteID, stratum = "1", wgt = finalwgt, 
                       xcoord = xmarinus, ycoord = ymarinus) %>%
    select(siteID, stratum, wgt, xcoord, ycoord)
  
  data.cont.ext <- select(finalData, siteID, parameter)
  
  subpopStatus <- cont.analysis(sites = sites.ext, subpop = subpop.ext, design = design.ext, data.cont = data.cont.ext, 
...
listOfSubpopulations <- function(finalData, parameterName){
  list(
    estimateVirginia = subpopEstimate(finalData,parameterName,NA, 'Virginia',NA),
    # By Basin
    estimateRoanoke = subpopEstimate(finalData,parameterName, 'Basin', 'Roanoke','Roanoke Basin'),
    estimateJames = subpopEstimate(finalData,parameterName, 'Basin', 'James','James Basin') ,
    estimatePotomacShenandoah = subpopEstimate(finalData,parameterName, 'Basin', 'Potomac-Shenandoah',NA) ,
    estimateRappahannockYork = subpopEstimate(finalData,parameterName, 'Basin', 'Rappahannock-York',NA),
    estimateNew = subpopEstimate(finalData,parameterName, 'Basin', 'New',NA) ,
    estimateChowan = subpopEstimate(finalData,parameterName, 'Basin', 'Chowan',NA), 
    estimateTennessee = subpopEstimate(finalData,parameterName, 'Basin', 'Tennessee',NA), 
    estimateHolston = subpopEstimate(finalData,parameterName, 'SubBasin', 'Holston',NA) ,
    estimateBigSandy = subpopEstimate(finalData,parameterName, 'SubBasin', 'Big Sandy',NA) ,
    estimateClinchPowell = subpopEstimate(finalData,parameterName, 'SubBasin', 'Clinch-Powell',NA) ,
    estimatePotomac = subpopEstimate(finalData,parameterName, 'SubBasin', 'Potomac',NA) ,
    estimateShenandoah = subpopEstimate(finalData,parameterName, 'SubBasin', 'Shenandoah',NA) ,
    estimateRappahannock = subpopEstimate(finalData,parameterName, 'SubBasin', 'Rappahannock',NA) ,
    estimateYork = subpopEstimate(finalData,parameterName, 'SubBasin', 'York',NA) ,
    # By Ecoregion
    estimatePiedmont = subpopEstimate(finalData,parameterName, 'EcoRegion', 'Piedmont',NA) ,
    estimateNorthernPiedmont = subpopEstimate(finalData,parameterName, 'EcoRegion', 'Northern Piedmont',NA) ,
    estimateCARV = subpopEstimate(finalData,parameterName, 'EcoRegion', 'Central Appalachian Ridges and Valleys',NA) ,
    estimateSEplains = subpopEstimate(finalData,parameterName, 'EcoRegion', 'Southeastern Plains',NA) ,
    estimateBRM = subpopEstimate(finalData,parameterName, 'EcoRegion', 'Blue Ridge Mountains',NA) ,
    estimateMountainBioregion = subpopEstimate(finalData,parameterName, 'BioRegion', 'Mountain','Mountain Bioregion'),
    estimatePiedmontBioregion = subpopEstimate(finalData,parameterName, 'BioRegion', 'Piedmont','Piedmont Bioregion') ,
    estimateCoastBioregion = subpopEstimate(finalData,parameterName, 'BioRegion', 'Coast', 'Coast Bioregion'),
    # By Order
    estimateFirstOrder = subpopEstimate(finalData,parameterName, 'Order', '1','First Order') ,
    estimateSecondOrder = subpopEstimate(finalData,parameterName, 'Order', '2','Second Order') ,
    estimateThirdOrder = subpopEstimate(finalData,parameterName, 'Order', '3','Third Order') ,
    estimateFourthOrder = subpopEstimate(finalData,parameterName, 'Order', '4','Fourth Order') ,
    estimateFifthOrder = subpopEstimate(finalData,parameterName, 'Order', '5','Fifth Order') ,
    # By Basin Size
    estimateBasin1 = subpopEstimate(finalData,parameterName, 'BasinSize', '1','<1 square mile') ,
    estimateBasin2 = subpopEstimate(finalData,parameterName, 'BasinSize', '2','1 to 10 square mile') ,
    estimateBasin3 = subpopEstimate(finalData,parameterName, 'BasinSize', '3','10 to 200 square mile') ,
    estimateBasin4 = subpopEstimate(finalData,parameterName, 'BasinSize', '4','>200 square mile') ,
    # By Year
    estimate2001 = subpopEstimate(finalData,parameterName, 'Year', '2001','Year 2001') ,
    estimate2002 = subpopEstimate(finalData,parameterName, 'Year', '2002','Year 2002'), 
    estimate2003 = subpopEstimate(finalData,parameterName, 'Year', '2003','Year 2003') ,
    estimate2004 = subpopEstimate(finalData,parameterName, 'Year', '2004','Year 2004') ,
    estimate2005 = subpopEstimate(finalData,parameterName, 'Year', '2005','Year 2005') ,
    estimate2006 = subpopEstimate(finalData,parameterName, 'Year', '2006','Year 2006') ,
    estimate2007 = subpopEstimate(finalData,parameterName, 'Year', '2007','Year 2007') ,
    estimate2008 = subpopEstimate(finalData,parameterName, 'Year', '2008','Year 2008') ,
    estimate2009 = subpopEstimate(finalData,parameterName, 'Year', '2009','Year 2009') ,
    estimate2010 = subpopEstimate(finalData,parameterName, 'Year', '2010','Year 2010') ,
    estimate2011 = subpopEstimate(finalData,parameterName, 'Year', '2011','Year 2011') ,
    estimate2012 = subpopEstimate(finalData,parameterName, 'Year', '2012','Year 2012') ,
    estimate2013 = subpopEstimate(finalData,parameterName, 'Year', '2013','Year 2013') ,
    estimate2014 = subpopEstimate(finalData,parameterName, 'Year', '2014','Year 2014') ,
    #estimate2015 = subpopEstimate(finalData,parameterName, 'Year', '2015','Year 2015') ,
    #estimate2016 = subpopEstimate(finalData,parameterName, 'Year', '2016','Year 2016') ,
    # By Phase
    estimatePhase1 = subpopEstimate(finalData,parameterName, 'Panel', 'Phase1','Phase One 2001-2007') ,
    estimatePhase2 = subpopEstimate(finalData,parameterName, 'Panel', 'Phase2','Phase Two 2008-2014') ,
    estimateBay = subpopEstimate(finalData,parameterName, 'BayShed', 'Bay','Bay Watersheds 2001-2014'),
    # Bay/ NonBay
    estimateNonBay = subpopEstimate(finalData,parameterName, 'BayShed', 'NonBay','Non-Bay Watersheds 2001-2014'),
    estimateBayPhase1 = subpopEstimate(finalData,parameterName, 'BayPanel', 'BayPhase1','Bay Watersheds 2001-2007'),
    estimateBayPhase2 = subpopEstimate(finalData,parameterName, 'BayPanel', 'BayPhase2','Bay Watersheds 2008-2014'),
    estimateNonBayPhase1 = subpopEstimate(finalData,parameterName, 'BayPanel', 'NonBayPhase1','Non-Bay Watersheds 2001-2007'),
    estimateNonBayPhase2 = subpopEstimate(finalData,parameterName, 'BayPanel', 'NonBayPhase2','Non-Bay Watersheds 2008-2014'),
    # Sampling Phase
    estimateBioPanelPhase1 = subpopEstimate(finalData,parameterName, 'BioPanel', 'Phase1','VSCI Scores 2001-2003'),
    estimateBioPanelPhase2 = subpopEstimate(finalData,parameterName, 'BioPanel', 'Phase2','VSCI Scores 2004-2006'),
    estimateBioPanelPhase3 = subpopEstimate(finalData,parameterName, 'BioPanel', 'Phase3','VSCI Scores 2007-2010'),
    estimateBioPanelPhase4 = subpopEstimate(finalData,parameterName, 'BioPanel', 'Phase4','VSCI Scores 2011-2014'),
    # IR window
    estimateIR2008 = subpopEstimate(finalData,parameterName, 'IR2008', '2008','IR2008'),
    estimateIR2010 = subpopEstimate(finalData,parameterName, 'IR2010', '2010','IR2010'),
    estimateIR2012= subpopEstimate(finalData,parameterName, 'IR2012', '2012','IR2012'),
    estimateIR2014 = subpopEstimate(finalData,parameterName, 'IR2014', '2014','IR2014'),
    estimateIR2016 = subpopEstimate(finalData,parameterName, 'IR2016', '2016','IR2016'),
    #estimateIR2018 = subpopEstimate(finalData,parameterName, 'IR2018', '2018','IR2018'),
    # Stream size
    estimateSmall = subpopEstimate(finalData,parameterName, 'StreamSizeCat','Small',NA),
    estimateMedium = subpopEstimate(finalData,parameterName, 'StreamSizeCat','Medium',NA),
    estimateLarge = subpopEstimate(finalData,parameterName, 'StreamSizeCat','Large',NA),
    # Stream size and sample phase
    estimatePhase1Small = subpopEstimate(finalData,parameterName, 'StreamSizeCatPhase','Phase1Small',NA),
    estimatePhase2Small = subpopEstimate(finalData,parameterName, 'StreamSizeCatPhase','Phase2Small',NA),
    estimatePhase1Medium = subpopEstimate(finalData,parameterName, 'StreamSizeCatPhase','Phase1Medium',NA),
    estimatePhase2Medium = subpopEstimate(finalData,parameterName, 'StreamSizeCatPhase','Phase2Medium',NA),
    estimatePhase1Large = subpopEstimate(finalData,parameterName, 'StreamSizeCatPhase','Phase1Large',NA),
    estimatePhase2Large = subpopEstimate(finalData,parameterName, 'StreamSizeCatPhase','Phase2Large',NA)
  )
}
