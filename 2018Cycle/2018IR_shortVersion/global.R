...
# HABITAT DATA
hab <- filter(dat,Subpopulation=='IR2018'&Indicator=='TotHab')%>%select(Value,Estimate.P,LCB95Pct.P,UCB95Pct.P)
...
# LRBS DATA
LRBS <- filter(dat,Subpopulation=='IR2018'&Indicator=='LRBS')%>%select(Value,Estimate.P,LCB95Pct.P,UCB95Pct.P)
totalsLRBS <- data.frame(Condition=c('Suboptimal','Fair','Optimal'),
                         pct=c(vlookup(-1,LRBS,2,TRUE), #suboptimal
                               vlookup(-0.5,LRBS,2,TRUE)-vlookup(-1,LRBS,2,TRUE), # fair
                               100-vlookup(-0.5,LRBS,2,TRUE)))%>%
...
#TN DATA
TN <- filter(dat,Subpopulation=='IR2018'&Indicator=='TN')%>%select(Value,Estimate.P,LCB95Pct.P,UCB95Pct.P)
...
#TP DATA
TP <- filter(dat,Subpopulation=='IR2018'&Indicator=='TP')%>%select(Value,Estimate.P,LCB95Pct.P,UCB95Pct.P)
...
# TDS DATA
TDS <- filter(dat,Subpopulation=='IR2018'&Indicator=='TDS')%>%select(Value,Estimate.P,LCB95Pct.P,UCB95Pct.P)
...
# METALS CCU DATA
mCCU <- filter(dat,Subpopulation=='IR2018'&Indicator=='MetalCCU')%>%select(Value,Estimate.P,LCB95Pct.P,UCB95Pct.P)
...
paramsummary$Parameter <- factor(paramsummary$Parameter,
                                 levels=c('Dissolved Oxygen','Ionic\nStrength',
                                          'Cumulative \nDissolved Metals','Total Nitrogen','pH\n(Below 6)','Habitat\nDisturbance',
