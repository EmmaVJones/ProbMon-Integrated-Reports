---
title: "How to Analyze Probabilistic Monitoring Data for Integrated Report Chapter"
author: "Emma Jones"
date: "March 2020"
...
This document walks you through the steps of calculating design weights and running analyses required for writing the Integrated Report chapter for the Probabilistic Monitoring Program due to EPA every two years. This document overviews the 2020 IR ProbMon chapter.

These methods are based off code originally written by Tony Olsen (EPA ORD), utilizing EPA's spsurvey package. This Virginia-specific version wraps those analyses into functions that efficiently processes CDF estimates for a number of selected parameters across various subpopulations of time windows and by basins, EPA level III ecoregions, 1:100k stream orders, and other spatial breakdowns. 

The procedure begins after data smashing of all bio/chem/landcover data. All data is from wadeable sites collected from 2001- 2018. Boatable methods will be included when appropriate biological indices are developed. 


### Data Organization

Bring in wadeable results (2001-2018).

```{r bio/chem/landcover data}
...
Quick detour explaining how to calculated Metals CCU results. 

```{r split out metals data for Metals CCU analysis}
...
rm(allDataMCCU); rm(metalsCCUresults)

allData <- read_csv('processedData/Wadeable_ProbMon_2001-2018.csv') # bring in metals ccu results
``` 


#### Survey Data

Survey data is a dataset we create of parameters we want to run CDF analyses on. We will first use this dataset to check for any metals exceedances of chronic, acute, or Public Water Supply (PWS) criteria.
...
Now time to find any exceedances of metals criteria for acute, chronic, or PWS.
...
You can review the results individually from the filter statements in the chunk above. These are included in the chapter manually at this point
...
The CDF analyses usually take between 2-3 hrs to run. That output is the input to the Prob Chapter .Rmd that generates the output word document with all text, tables, and graphics summarizing the results generated from this script.


### Contacts

Please contact Jason Hill (jason.hill@deq.virginia.gov) and Emma Jones (emma.jones@deq.virginia.gov) with any questions regarding the methods outlined above.
